CREATE TABLE target AS SELECT 28123 as n;
...
CREATE TABLE abundant AS
SELECT n from divsums WHERE d > n;

WITH abundant_sums(value) AS (
	SELECT DISTINCT a.n + b.n
	FROM abundant a
	CROSS JOIN abundant b
	WHERE a.n + b.n <= (SELECT n FROM target)
)
SELECT (SELECT n*(n + 1)/2 FROM target) - SUM(value) FROM abundant_sums;
